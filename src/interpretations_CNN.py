...
class Interpretation_Generator_CNN:

    """
    Builds a Bayesian Causal Graph from a deep CNN model and performs causal intervention 
    ...
    def train_model_CNN(self, epochs, batch_size):
    ...
    def interpret_CNN(self, labels, layer, label):
        ...
        for i in range(len(self.model.layers)):
            ...
            #print(intermediate_output.reshape((-1, intermediate_output.shape[2]*intermediate_output.shape[3])).shape)
            #print(self.X_test.shape)
            if intermediate_output.shape[1] == 1:
                try:
                    self.data = np.concatenate((self.data, intermediate_output.reshape((-1, intermediate_output.shape[2]*intermediate_output.shape[3]))), axis = 1)
                except:
                    self.data = np.concatenate((self.X_test.reshape((-1, self.X_test.shape[2]*self.X_test.shape[3])), intermediate_output.reshape((-1, intermediate_output.shape[2]*intermediate_output.shape[3]))), axis = 1)
            
            
        self.data = np.concatenate((self.data, self.y_pred.reshape(len(self.y_pred), 1)), axis = 1)
        
        if layer == 0:
            layer_end = self.X_test.shape[2]*self.X_test.shape[3]
            layer_start = 0
            values = range(self.X_test.shape[2]*self.X_test.shape[3])
            
        else:
            layer_start = 0
            layer_start += self.X_test.shape[2]*self.X_test.shape[3]
            for i in range(len(self.model.layers[:layer-1])):
                layer_start += self.model.layers[i].output.shape[2]*self.model.layers[i].output.shape[3]
            
            layer_end = layer_start + self.model.layers[layer-1].output.shape[2]*self.model.layers[layer-1].output.shape[3]
            values = range(self.model.layers[layer-1].output.shape[2]*self.model.layers[layer-1].output.shape[3])
    ...
    def perturbation_tester_CNN(self, SubModel,label, layer = 0):
